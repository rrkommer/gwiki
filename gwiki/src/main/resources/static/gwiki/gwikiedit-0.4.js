	...
	$.fn.gwikiedit = function(settings) {
		...
		return this.each(function() {
			...
			var editdiv = $("#gwikiwed" + pn).wrap(
			    "<div id=\"" + "gwikiwed2" + oId + "\" title=\"" + oTitle + "\" class=\"" + oClassName
			        + "\" width=\"100%\" height=\"100%\"></div>");

			// add the toolbar et statusbar
			var toolbar = $("<table class='gwikiedit_table'><tbody><tr></tr></tbody></table>");
			var toolbarWrapper = $("<div class=\"" + options.toolBarClassName + "\"></div>").insertBefore("#gwikiwed" + pn);
			...
			$(options.toolbars['wikitb']).each(
			    function(i) {
				    ...
				    var clazz = button.type != null && button.type == "seperator" ? "gwikiedit_seperator" : "gwikiedit_icon "
				        + button.clazz;
				    var output = $(
				        "<td><a href=\"\" accesskey=\"" + button.accesskey + "\" " + " bodytag=\"" + hasBody + "\" title=\""
				            + title + "\" class='" + clazz + "'></a></td>").click(function() {
					    tag(button);
					    return false;
				    });
				    // var output = $(
				    // "<td><span accesskey=\"" + button.accesskey + "\" "
				    // + " bodytag=\"" + hasBody
			...
			function insertTemplate() {
				...
				if (url.length == 0 || url.indexOf("/", url.length - 1) == -1) {
					url += "/";
				}
