...
public class SardineImpl implements Sardine
{
  ...
  /** */
  public SardineImpl(Factory factory, String username, String password, SSLSocketFactory sslSocketFactory, HttpRoutePlanner routePlanner)
  ...
  public List<DavResource> getResources(String url) throws SardineException
  {
    ...
    if (!SardineUtil.isGoodResponse(statusLine.getStatusCode())) {
      propFind.abort();
      throw new SardineException("Failed to get resources. Is the url valid?", url, statusLine.getStatusCode(), statusLine
          .getReasonPhrase());
    }
    ...
    for (Response resp : responses) {
      ...
      // rrk next line:
      // org: if (isDirectory && (contentType == null))
      // patched:
      ...
      DavResource dr = new DavResource(hostPart + baseUrl, name, SardineUtil.parseDate(creationdate), SardineUtil.parseDate(modifieddate),
          contentType, Long.valueOf(contentLength), currentDirectory, customProps);
  ...
  public void setCustomProps(String url, Map<String, String> setProps, List<String> removeProps) throws SardineException
  {
    ...
    if (!SardineUtil.isGoodResponse(statusLine.getStatusCode())) {
      propPatch.abort();
      throw new SardineException("Failed to set custom properties on resources.", url, statusLine.getStatusCode(), statusLine
          .getReasonPhrase());
    }
  ...
  public void move(String sourceUrl, String destinationUrl) throws SardineException
  {
    ...
    if (!SardineUtil.isGoodResponse(statusLine.getStatusCode())) {
      move.abort();
      throw new SardineException("sourceUrl: " + sourceUrl + ", destinationUrl: " + destinationUrl, statusLine.getStatusCode(), statusLine
          .getReasonPhrase());
    }
  ...
  private HttpResponse executeWrapper(HttpRequestBase base) throws SardineException
  {
    try {
      if (this.authEnabled) {
        ...
        String value = "Basic "
            + new String(Base64.encodeBase64(new String(creds.getUserPrincipal().getName() + ":" + creds.getPassword()).getBytes()));
